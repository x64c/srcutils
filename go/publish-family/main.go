...
func usage() {
	...
	stderr("ONE version, living in one umbrella git repo (workspace root = git root).\n\n")
	stderr("usage:\n")
	stderr("  publish-family -conf <file> [-version vX.Y.Z] [-dry]\n\n")
	stderr("The conf declares STRUCTURE only: repo, modprefix, commitmsg, goenv, and the\n")
	stderr("EXPLICIT module dependency graph (dir -> family deps). Everything per-run is\n")
